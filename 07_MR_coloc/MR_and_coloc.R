...
MR_and_coloc <- function (SMRfile,trait_name,plotpath,
  ...
  path<-paste0(plotpath,'/',trait_name,'_',probe,'.SMRLocusPlot.pdf')
  
  pdf(path,width = 12,height = 8)
  SMRLocusPlot(data=SMRData, smr_thresh=0.01, heidi_thresh=0.05, plotWindow=500, max_anno_probe=16)
  dev.off()
  ...
  MRinput$samplesize.exposure<-n_qtl
  # MRinput$r.outcome<-get_r_from_lor(MRinput$beta.outcome,MRinput$eaf.outcome,ncase=25060,ncontrol=449978,prevalence=0.1)
  # 
  df_MR_hetero <- mr_heterogeneity(MRinput)  
  df_MR_pleio <- mr_pleiotropy_test(MRinput) 
  
  steiger_test<-directionality_test(MRinput)
  
  table[['hetero']]<-df_MR_hetero
  table[['plei']]<-df_MR_pleio
  table[['Steiger']]<-steiger_test
  ...
  path<-paste0(plotpath,'/',trait_name,'_',probe,'.Twosample_MR_Plot.pdf')
  
  pdf(path,width = 15,height = 10)
  print(wrap_plots(plot,nrow=2) )
  dev.off()
  
  } else {'Missing instrumental variables, skip twosample MR analysis'}
  ####################
  #coloc
  ...
  path<-paste0(plotpath,'/',trait_name,'_',probe,'.sensitivity.pdf')
  pdf(path,width = 12,height = 8)
  ...
  path<-paste0(plotpath,'/',trait_name,'_',probe,'.fig_region_stack.pdf')
  ...
  fig_region_stack(
    data = assoc,
    traits = c(trait_name, probe),
    corr = corr,
    build = g_version,
    highlights = highlights,
    title_center = TRUE)
  
  p[[2]]<-ggplot(data = assoc,aes(x=-log10(pvalue_1),y=-log10(pvalue_2)))+
    geom_pointdensity(adjust = 4,show.legend =T)+
    scale_color_viridis()+ 
    scale_colour_gradient(low = "#f2c88f", high = "#447fdd")+
    theme_classic()+
    ylab('-log10(P value) eQTL')+
    xlab('-log10(P value) GWAS')+
    theme_bw() +
